...
import { getTalentDivisionBySlug } from "@/data/talents.divisions";
import { formatTalentsPrice } from "@/data/talents.products";

import { useTalentsCart } from "./TalentsCartProvider";

export default function TalentsCartPage() {
  const {
    clearCart,
    decrementItem,
    incrementItem,
    ...
    () =>
      new Map(
        items.map((item) => [
          ...
          getTalentDivisionBySlug(item.product.division)?.name ??
            item.product.division,
        ]),
      ),
    [items],
      ...
      <div className="border-b border-bg/10 pb-8">
        ...
        <div className="mt-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-4 font-sans text-[0.65rem] uppercase tracking-[0.22em] text-bg/48">
            ...
            <span className="h-3 w-px bg-bg/12" />
            <span>Activation de visibilité</span>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white">
          ...
          <div className="space-y-0">
            {items.map((item) => {
              ...
              const divisionName =
                divisionLabels.get(item.product.division) ??
                item.product.division;
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="grid grid-cols-[72px_minmax(0,1fr)] gap-4 sm:grid-cols-[88px_minmax(0,1fr)] sm:gap-5">
                        <Link
                          href={`/talents-agency/${item.product.division}/${item.product.id}`}
