...
export async function generateMetadata({
  ...
  const context = getProductionArtistStorefrontContext(artist, "drum-kits");
...
export default async function ProductionArtistDrumKitsPage({
  params,
}: PageProps) {
  const { artist } = await params;
  const context = getProductionArtistStorefrontContext(artist, "drum-kits");
