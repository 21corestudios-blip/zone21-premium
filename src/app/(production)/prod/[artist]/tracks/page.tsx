...
export async function generateMetadata({
  ...
  const context = getProductionArtistStorefrontContext(artist, "tracks");
...
export default async function ProductionArtistTracksPage({ params }: PageProps) {
  const { artist } = await params;
  const context = getProductionArtistStorefrontContext(artist, "tracks");
